 ...
 * to set a desired temperature. The MSP430F5529 uses this information to automate a PWM signal that drives a fan and
...
unsigned char Q ;                                   // used in ___ to keep track of ____
double prevError = 0;                               // used in ___ to calculate D and I for PID control
double error;                                       // used in ___ to calculate closed loop correction
unsigned int watch;                                 // used in ___ to monitor the value of TA0CCR1
int pwm;                                            // used in ___ to calculate _____
double qin;                                         // used in ___ to calculate _____
double I = 0;                                       // used in ___ to calculate "integral" in PID control
double D = 0;                                       // used in ___ to calculate "derivative" in PID control
double P = 0;                                       // used in ___ to calculate "proportion" in PID control
...
int main(void)
{
    ...
    while (1)
    {
        ...
        pwm = TA0CCR0*P+D*5+95*I;                   // PID control
...
/*---------BINARY TO TEMPERATURE FUNCTION---------*/
